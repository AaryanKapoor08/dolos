 ...
 *       Phase 3C);
 *   <li>{@code workflow} — the BPMN investigation workflow (Flowable, Phase 3D);
 *   <li>{@code integration} — Kafka in/out: {@code AlertRaised} → {@code OpenCase} plus the Modulith
 *       event-publication outbox publishing case events (Phase 3E).
 * </ul>
 *
 ...
 *
 * <p>{@code @EnableAsync} backs the Modulith {@code @ApplicationModuleListener} outbox: the case-event
 * publications are dispatched to Kafka asynchronously after the publishing transaction commits.
 *
